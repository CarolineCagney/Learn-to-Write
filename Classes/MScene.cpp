...
Scene* MScene::createScene()//Here is where the scene is created
{
	// 'scene' is an autorelease object
	auto scene = Scene::create();//creating the scene with added physcis engine 
	auto layer = MScene::create();//creating the game layer 
...
bool MScene::init()//initing the game so the scene can be made 
{
	//////////////////////////////
	// 1. super init first
	if (!CCLayer::init())
	...
	auto backgroundSprite = Sprite::create("background13.png");//The image for the background is gotten here
	backgroundSprite->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(backgroundSprite);

	auto MainMenu = MenuItemImage::create("MainMenu.png", "MainMenu.png", CC_CALLBACK_1(MScene::GoToMainMenuScene, this));
	MainMenu->setPosition(Point(winSize.width / 4 + origin.x, winSize.height / 9 + origin.y));

	_m = Sprite::create("Letterm.png");
	_m->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 1.5 + origin.y));
	this->addChild(_m);
	...
	dot->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 1.4 + origin.y));
	this->addChild(dot);

	dot2 = Sprite::create("bluedot.png");
	dot2->setPosition(Point(winSize.width / 2.6 + origin.x, winSize.height / 1.4 + origin.y));
	this->addChild(dot2);

	dot3 = Sprite::create("blackdot.png");
	dot3->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2.2 + origin.y));
	//2.6 -> 2
	this->addChild(dot3);

	dot4 = Sprite::create("orangedot.png");
	dot4->setPosition(Point(winSize.width / 1.62 + origin.x, winSize.height / 1.4 + origin.y));
	this->addChild(dot4);

	dot5 = Sprite::create("purpledot.png");
	dot5->setPosition(Point(winSize.width / 2.6 + origin.x, winSize.height / 2.2 + origin.y));
	this->addChild(dot5);

	dot6 = Sprite::create("pinkdot.png");
	dot6->setPosition(Point(winSize.width / 1.62 + origin.x, winSize.height / 2.2 + origin.y));
	//2.6 -> 2
	this->addChild(dot6);

/*	dot7 = Sprite::create("lightbluedot.png");
	dot7->setPosition(Point(winSize.width / 2.5 + origin.x, winSize.height / 1.4 + origin.y));
	//1.6 -> 1
	this->addChild(dot7);

	dot8 = Sprite::create("browndot.png");
	dot8->setPosition(Point(winSize.width / 2.2 + origin.x, winSize.height / 1.8 + origin.y));
	this->addChild(dot8);

	dot9 = Sprite::create("whitedot.png");
	dot9->setPosition(Point(winSize.width / 2.4 + origin.x, winSize.height / 2.2 + origin.y));
	this->addChild(dot9);
*/
	auto touchListener = EventListenerTouchOneByOne::create();

	touchListener->onTouchBegan = CC_CALLBACK_2(MScene::onTouchBegan, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(MScene::onTouchEnded, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(MScene::onTouchMoved, this);
...
bool MScene::onTouchBegan(Touch* touch, Event* event)
{
...
void MScene::onTouchEnded(Touch* touch, Event* event)
{
	/*
...
void MScene::onTouchMoved(Touch* touch, Event* event)
{
	/*
	...
	*/
	...
	if (_m->getBoundingBox().containsPoint(touch->getLocation()))
	{
...
void MScene::GoToWinScene(Ref *sender)//go to game scene method implementation 
{
...
void MScene::GoToLoseScene(Ref *sender)//go to game scene method implementation 
{
...
void MScene::GoToMainMenuScene(Ref *sender)//go to game scene method implementation 
{
