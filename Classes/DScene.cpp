...
Scene* DScene::createScene()//Here is where the scene is created
{
	// 'scene' is an autorelease object
	auto scene = Scene::create();//creating the scene with added physcis engine 
	auto layer = DScene::create();//creating the game layer 
...
bool DScene::init()//initing the game so the scene can be made 
{
	//////////////////////////////
	// 1. super init first
	if (!CCLayer::init())
	...
	*/
	auto MainMenu = MenuItemImage::create("MainMenu.png", "MainMenu.png", CC_CALLBACK_1(DScene::GoToMainMenuScene, this));
	//The position for the background is set here
	...
	_d = Sprite::create("Letterd.png");
	_d->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(_d);

	dot = Sprite::create("dot.png");
	dot->setPosition(Point(winSize.width / 1.75 + origin.x, winSize.height / 1.4 + origin.y));
	this->addChild(dot);

	dot2 = Sprite::create("purpledot.png");
	dot2->setPosition(Point(winSize.width / 2.35 + origin.x, winSize.height / 1.8 + origin.y));
	this->addChild(dot2);

	dot3 = Sprite::create("bluedot.png");
	dot3->setPosition(Point(winSize.width / 2.35 + origin.x, winSize.height / 3.1 + origin.y));
	this->addChild(dot3);

	dot4 = Sprite::create("orangedot.png");
	dot4->setPosition(Point(winSize.width / 1.75 + origin.x, winSize.height / 3.7 + origin.y));
	this->addChild(dot4);

	dot5 = Sprite::create("dot.png");
	dot5->setPosition(Point(winSize.width / 1.8 + origin.x, winSize.height / 1.8 + origin.y));
	this->addChild(dot5);
	...
	*/
	...
	touchListener->onTouchBegan = CC_CALLBACK_2(DScene::onTouchBegan, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(DScene::onTouchEnded, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(DScene::onTouchMoved, this);
...
bool DScene::onTouchBegan(Touch* touch, Event* event)
{
...
void DScene::onTouchEnded(Touch* touch, Event* event)
{
	/*
...
void DScene::onTouchMoved(Touch* touch, Event* event)
{
	/*
	...
	*/
	...
	if (_d->getBoundingBox().containsPoint(touch->getLocation()))
	{
...
void DScene::GoToWinScene(Ref *sender)//go to game scene method implementation 
{
...
void DScene::GoToLoseScene(Ref *sender)//go to game scene method implementation 
{
...
void DScene::GoToMainMenuScene(Ref *sender)//go to game scene method implementation 
{
