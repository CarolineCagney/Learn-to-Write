...
Scene* JScene::createScene()//create scene
{
	// 'scene' is an autorelease object
	auto scene = Scene::create();//creating the scene with added physcis engine 
	auto layer = JScene::create();//creating the game layer 
...
bool JScene::init()//initing the game so the scene can be made 
{
	//////////////////////////////
	// 1. super init first
	if (!CCLayer::init())
	...
	auto backgroundSprite = Sprite::create("background10.png");
	...
	*/
	auto MainMenu = MenuItemImage::create("MainMenu.png", "MainMenu.png", CC_CALLBACK_1(JScene::GoToMainMenuScene, this));
	//The position for the background is set here
	...
	_j = Sprite::create("Letterj.png");
	_j->setPosition(Point(winSize.width / 1.9 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(_j);
	...
	dot->setPosition(Point(winSize.width / 1.85 + origin.x, winSize.height / 1.29 + origin.y));
	this->addChild(dot);

	dot2 = Sprite::create("dot.png");
	dot2->setPosition(Point(winSize.width / 1.85 + origin.x, winSize.height / 1.85 + origin.y));
	this->addChild(dot2);

	dot3 = Sprite::create("dot.png");
	dot3->setPosition(Point(winSize.width / 1.85 + origin.x, winSize.height / 2.5 + origin.y));
	this->addChild(dot3);

	dot4 = Sprite::create("dot.png");
	dot4->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 5 + origin.y));
	this->addChild(dot4);

	dot5 = Sprite::create("dot.png");
	dot5->setPosition(Point(winSize.width / 1.85 + origin.x, winSize.height / 3.5 + origin.y));
	this->addChild(dot5);

	auto touchListener = EventListenerTouchOneByOne::create();

	touchListener->onTouchBegan = CC_CALLBACK_2(JScene::onTouchBegan, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(JScene::onTouchEnded, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(JScene::onTouchMoved, this);
...
bool JScene::onTouchBegan(Touch* touch, Event* event)
{
...
void JScene::onTouchEnded(Touch* touch, Event* event)
{
	/*
...
void JScene::onTouchMoved(Touch* touch, Event* event)
{
	/*
	...
	*/
	...
	if (_j->getBoundingBox().containsPoint(touch->getLocation()))
	{
...
void JScene::GoToWinScene(Ref *sender)//go to game scene method implementation 
{
...
void JScene::GoToLoseScene(Ref *sender)//go to game scene method implementation 
{
...
void JScene::GoToMainMenuScene(Ref *sender)//go to game scene method implementation 
{
