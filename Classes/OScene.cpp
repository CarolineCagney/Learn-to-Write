...
Scene* OScene::createScene()//Here is where the scene is created
{
	// 'scene' is an autorelease object
	auto scene = Scene::create();//creating the scene with added physcis engine 
	auto layer = OScene::create();//creating the game layer 
...
bool OScene::init()//initing the game so the scene can be made 
{
	//////////////////////////////
	// 1. super init first
	if (!CCLayer::init())
	...
	auto backgroundSprite = Sprite::create("background15.png");//The image for the background is gotten here
	backgroundSprite->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(backgroundSprite);

	auto MainMenu = MenuItemImage::create("MainMenu.png", "MainMenu.png", CC_CALLBACK_1(OScene::GoToMainMenuScene, this));
	MainMenu->setPosition(Point(winSize.width / 4 + origin.x, winSize.height / 9 + origin.y));

	_o = Sprite::create("Lettero.png");
	_o->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(_o);
	...
	dot2->setPosition(Point(winSize.width / 2 + origin.x, 200 + origin.y));
	this->addChild(dot2);

	dot3 = Sprite::create("purpledot.png");
	dot3->setPosition(Point(winSize.width / 2.5 + origin.x, winSize.height / 1.8 + origin.y));
	this->addChild(dot3);

	dot4 = Sprite::create("orangedot.png");
	dot4->setPosition(Point(800 + origin.x, 400 + origin.y));
	this->addChild(dot4);

	auto touchListener = EventListenerTouchOneByOne::create();

	touchListener->onTouchBegan = CC_CALLBACK_2(OScene::onTouchBegan, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(OScene::onTouchEnded, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(OScene::onTouchMoved, this);
...
bool OScene::onTouchBegan(Touch* touch, Event* event)
{
...
void OScene::onTouchEnded(Touch* touch, Event* event)
{
	/*
...
void OScene::onTouchMoved(Touch* touch, Event* event)
{
	/*
	...
	*/
	...
	if (_o->getBoundingBox().containsPoint(touch->getLocation()))
	{
...
void OScene::GoToWinScene(Ref *sender)//go to game scene method implementation 
{
...
void OScene::GoToLoseScene(Ref *sender)//go to game scene method implementation 
{
...
void OScene::GoToMainMenuScene(Ref *sender)//go to game scene method implementation 
{
