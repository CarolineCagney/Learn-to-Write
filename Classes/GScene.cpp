...
Scene* GScene::createScene()//create scene
{
	// 'scene' is an autorelease object
	auto scene = Scene::create();//creating the scene with added physcis engine 
	auto layer = GScene::create();//creating the game layer 
...
bool GScene::init()//initing the game so the scene can be made 
{
	//////////////////////////////
	// 1. super init first
	if (!CCLayer::init())
	...
	auto backgroundSprite = Sprite::create("background7.png");// main menu image 
	backgroundSprite->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(backgroundSprite);

	auto MainMenu = MenuItemImage::create("MainMenu.png", "MainMenu.png", CC_CALLBACK_1(GScene::GoToMainMenuScene, this));
	MainMenu->setPosition(Point(winSize.width / 4 + origin.x, winSize.height / 9 + origin.y));

	_g = Sprite::create("Letterg.png");
	_g->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(_g);
	...
	dot->setPosition(Point(winSize.width / 2  + origin.x, winSize.height / 1.3 + origin.y));
	this->addChild(dot);

	dot2 = Sprite::create("blackdot.png");
	dot2->setPosition(Point(winSize.width / 1.7 + origin.x, winSize.height / 2.4 + origin.y));
	this->addChild(dot2);

	dot3 = Sprite::create("orangedot.png");
	dot3->setPosition(Point(winSize.width / 1.7 + origin.x, winSize.height / 1.5 + origin.y));
	this->addChild(dot3);

	dot4 = Sprite::create("blackdot.png");
	dot4->setPosition(Point(winSize.width / 2.3 + origin.x, 200 + origin.y));
	this->addChild(dot4);

	dot5 = Sprite::create("orangedot.png");
	dot5->setPosition(Point(winSize.width / 2.3 + origin.x, 320 + origin.y));
	this->addChild(dot5);

	auto touchListener = EventListenerTouchOneByOne::create();

	touchListener->onTouchBegan = CC_CALLBACK_2(GScene::onTouchBegan, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(GScene::onTouchEnded, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(GScene::onTouchMoved, this);
...
bool GScene::onTouchBegan(Touch* touch, Event* event)
{
...
void GScene::onTouchEnded(Touch* touch, Event* event)
{
	/*
...
void GScene::onTouchMoved(Touch* touch, Event* event)
{
	/*
	...
	*/
	...
	if (_g->getBoundingBox().containsPoint(touch->getLocation()))
	{
...
void GScene::GoToWinScene(Ref *sender)//go to game scene method implementation 
{
...
void GScene::GoToLoseScene(Ref *sender)//go to game scene method implementation 
{
...
void GScene::GoToMainMenuScene(Ref *sender)//go to game scene method implementation 
{
