...
Scene* KScene::createScene()//Here is where the scene is created
{
	// 'scene' is an autorelease object
	auto scene = Scene::create();//creating the scene with added physcis engine 
	auto layer = KScene::create();//creating the game layer 
...
bool KScene::init()//initing the game so the scene can be made 
{
	//////////////////////////////
	// 1. super init first
	if (!CCLayer::init())
	...
	auto backgroundSprite = Sprite::create("background11.png");//The image for the background is gotten here
	backgroundSprite->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(backgroundSprite);

	auto MainMenu = MenuItemImage::create("MainMenu.png", "MainMenu.png", CC_CALLBACK_1(KScene::GoToMainMenuScene, this));
	MainMenu->setPosition(Point(winSize.width / 4 + origin.x, winSize.height / 9 + origin.y));

	_k = Sprite::create("Letterk.png");
	_k->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(_k);
	...
	dot2->setPosition(Point(750 + origin.x, 400 + origin.y));
	this->addChild(dot2);
	...
	dot4->setPosition(Point(750 + origin.x, winSize.height / 4 + origin.y));
	this->addChild(dot4);

	auto touchListener = EventListenerTouchOneByOne::create();

	touchListener->onTouchBegan = CC_CALLBACK_2(KScene::onTouchBegan, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(KScene::onTouchEnded, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(KScene::onTouchMoved, this);
...
bool KScene::onTouchBegan(Touch* touch, Event* event)
{
...
void KScene::onTouchEnded(Touch* touch, Event* event)
{
	/*
...
void KScene::onTouchMoved(Touch* touch, Event* event)
{
	/*
	...
	*/
	...
	if (_k->getBoundingBox().containsPoint(touch->getLocation()))
	{
...
void KScene::GoToWinScene(Ref *sender)//go to game scene method implementation 
{
...
void KScene::GoToLoseScene(Ref *sender)//go to game scene method implementation 
{
...
void KScene::GoToMainMenuScene(Ref *sender)//go to game scene method implementation 
{
