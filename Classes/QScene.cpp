...
Scene* QScene::createScene()//Here is where the scene is created
{
	// 'scene' is an autorelease object
	auto scene = Scene::create();//creating the scene with added physcis engine 
	auto layer = QScene::create();//creating the game layer 
...
bool QScene::init()//initing the game so the scene can be made 
{
	//////////////////////////////
	// 1. super init first
	if (!CCLayer::init())
	...
	auto backgroundSprite = Sprite::create("background17.png");
	...
	*/
	auto MainMenu = MenuItemImage::create("MainMenu.png", "MainMenu.png", CC_CALLBACK_1(QScene::GoToMainMenuScene, this));
	//The position for the background is set here
	...
	_q = Sprite::create("Letterq.png");
	_q->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(_q);
	...
	dot->setPosition(Point(winSize.width / 2.2 + origin.x, winSize.height / 1 + origin.y));
	this->addChild(dot);

	dot2 = Sprite::create("blackdot.png");
	dot2->setPosition(Point(winSize.width / 2.1 + origin.x, winSize.height / 1.1 + origin.y));
	this->addChild(dot2);

	auto touchListener = EventListenerTouchOneByOne::create();

	touchListener->onTouchBegan = CC_CALLBACK_2(QScene::onTouchBegan, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(QScene::onTouchEnded, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(QScene::onTouchMoved, this);
...
bool QScene::onTouchBegan(Touch* touch, Event* event)
{
...
void QScene::onTouchEnded(Touch* touch, Event* event)
{
	/*
...
void QScene::onTouchMoved(Touch* touch, Event* event)
{
	/*
	...
	*/
	...
	if (_q->getBoundingBox().containsPoint(touch->getLocation()))
	{
...
void QScene::GoToWinScene(Ref *sender)//go to game scene method implementation 
{
...
void QScene::GoToLoseScene(Ref *sender)//go to game scene method implementation 
{
...
void QScene::GoToMainMenuScene(Ref *sender)//go to game scene method implementation 
{
