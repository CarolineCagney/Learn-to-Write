...
Scene* IScene::createScene()//Here is where the scene is created
{
	// 'scene' is an autorelease object
	auto scene = Scene::create();//creating the scene with added physcis engine 
	auto layer = IScene::create();//creating the game layer 
...
bool IScene::init()//initing the game so the scene can be made 
{
	//////////////////////////////
	// 1. super init first
	if (!CCLayer::init())
	...
	auto backgroundSprite = Sprite::create("background9.png");
	...
	*/
	auto MainMenu = MenuItemImage::create("MainMenu.png", "MainMenu.png", CC_CALLBACK_1(IScene::GoToMainMenuScene, this));
	//The position for the background is set here
	...
	_i = Sprite::create("Letteri.png");
	_i->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 2 + origin.y));
	this->addChild(_i);

	dot = Sprite::create("dot.png");
	dot->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 1.38 + origin.y));
	this->addChild(dot);

	dot2 = Sprite::create("dot.png");
	dot2->setPosition(Point(winSize.width / 2 + origin.x, winSize.height / 1.9 + origin.y));
	this->addChild(dot2);

	dot3 = Sprite::create("dot.png");
	dot3->setPosition(Point(winSize.width / 2. + origin.x, winSize.height / 2.5 + origin.y));
	this->addChild(dot3);
	...
	touchListener->onTouchBegan = CC_CALLBACK_2(IScene::onTouchBegan, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(IScene::onTouchEnded, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(IScene::onTouchMoved, this);
...
bool IScene::onTouchBegan(Touch* touch, Event* event)
{
...
void IScene::onTouchEnded(Touch* touch, Event* event)
{
	/*
...
void IScene::onTouchMoved(Touch* touch, Event* event)
{
	/*
	...
	*/
	...
	if (_i->getBoundingBox().containsPoint(touch->getLocation()))
	{
...
void IScene::GoToWinScene(Ref *sender)//go to game scene method implementation 
{
...
void IScene::GoToLoseScene(Ref *sender)//go to game scene method implementation 
{
...
void IScene::GoToMainMenuScene(Ref *sender)//go to game scene method implementation 
{
